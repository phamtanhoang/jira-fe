...
export function ActivityFeed({ issueId }: { issueId: string }) {
  ...
  if (isLoading) return <div className="text-[12px] text-muted-foreground">{t("activityFeed.loading")}</div>;
  if (!activities?.length) return <div className="text-[12px] text-muted-foreground">{t("activityFeed.noActivity")}</div>;
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[12px]">
              ...
              <span className="text-muted-foreground">
                {ACTION_LABEL_KEYS[a.action] ? t(ACTION_LABEL_KEYS[a.action]) : a.action.toLowerCase()}
              </span>
              {a.field && (
              ...
              {a.oldValue && a.newValue && (
                <>
                  {" "}
                  <span className="line-through text-muted-foreground/60">{a.oldValue}</span>
                  {" → "}
